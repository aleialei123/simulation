...
const (
	...
	GenerateFabricConfig                  = "GenerateFabricConfig"
...
func (t *Topology) GenerateNodes() error {
	...
	for _, nodeParam := range t.TopologyParams.Nodes {
		...
		case types.NetworkNodeType_FabricPeerNode:
			fmt.Println("hello2")
			fabricNode := nodes.NewFabricPeerNode(nodeParam.Index, nodeParam.X, nodeParam.Y)
			fmt.Println("hello2")
			t.FabricPeerNodes = append(t.FabricPeerNodes, fabricNode)
			fmt.Println("hello2")
			// 注意只能创建一次
			abstractFabricPeerNode := node.NewAbstractNode(types.NetworkNodeType_FabricPeerNode, fabricNode, TopologyInstance.TopologyGraph)
			fmt.Println("hello2")
			t.FabricPeerAbstractNodes = append(t.FabricPeerAbstractNodes, abstractFabricPeerNode)
			fmt.Println("hello2")
			t.AllAbstractNodes = append(t.AllAbstractNodes, abstractFabricPeerNode)
			fmt.Println("hello2")
			t.AbstractNodesMap[fabricNode.ContainerName] = abstractFabricPeerNode
			fmt.Println("hello2")
		case types.NetworkNodeType_FabricOrdererNode:
			fabricNode := nodes.NewFabricOrdererNode(nodeParam.Index, nodeParam.X, nodeParam.Y)
			t.FabricOrdererNodes = append(t.FabricOrdererNodes, fabricNode)
			// 注意只能创建一次
			abstractFabricOrdererNode := node.NewAbstractNode(types.NetworkNodeType_FabricOrdererNode, fabricNode, TopologyInstance.TopologyGraph)
			t.FabricOrdererAbstractNodes = append(t.FabricOrdererAbstractNodes, abstractFabricOrdererNode)
			t.AllAbstractNodes = append(t.AllAbstractNodes, abstractFabricOrdererNode)
			t.AbstractNodesMap[fabricNode.ContainerName] = abstractFabricOrdererNode
		}
	}
...
func (t *Topology) getSourceNodeAndTargetNode(sourceNodeParam, targetNodeParam params.NodeParam) (*node.AbstractNode, *node.AbstractNode, error) {
	...
	case types.NetworkNodeType_FabricOrdererNode:
		sourceNode = t.FabricOrdererAbstractNodes[sourceNodeParam.Index-1]
	default:
	...
	case types.NetworkNodeType_FabricOrdererNode:
		targetNode = t.FabricOrdererAbstractNodes[targetNodeParam.Index-1]
	default:
...
// GenerateFabricConfig 进行Fabric配置文件的生成
func (t *Topology) GenerateFabricConfig() error {
	...
	fabricOrdererNodeCount := len(t.FabricOrdererNodes)
	fabricPeerNodeCount := len(t.FabricPeerNodes)
	// if fabricOrdererNodeCount == 0 {
	// 	topologyLogger.Infof("no fabricOrderer nodes -> not generate")
	// 	return nil
	// }
	// if fabricPeerNodeCount == 0 {
	// 	topologyLogger.Infof("no fabricPeer nodes -> not generate")
	// 	return nil
	// }
	if fabricOrdererNodeCount == 0 && fabricPeerNodeCount == 0 {
		topologyLogger.Infof("no fabric nodes -> not generate")
		return nil
	}
	ipv4AddressesPeer := t.GetFabricPeerNodeListenAddresses()
	ipv4AddressesOrderer := t.GetFabricOrdererNodeListenAddresses()
	fabricPrepare := fabric_prepare.NewFabricPrepare(fabricOrdererNodeCount, fabricPeerNodeCount, ipv4AddressesOrderer, ipv4AddressesPeer)
...
func (t *Topology) GeneratePortMapping() error {
	...
	portMapping, portFabricOrdererMapping, portFabricPeerMapping, err := t.GetContainerNameToPortMapping()
	if err != nil {
	...
	for key, value := range portFabricOrdererMapping {
		finalString += fmt.Sprintf("%s->%d->%d\n", key, value.p2pPort, value.rpcPort)
	}
	for key, value := range portFabricPeerMapping {
		finalString += fmt.Sprintf("%s->%d->%d\n", key, value.p2pPort, value.rpcPort)
	}
	// 进行所有节点的遍历
	for _, abstractNode := range t.AllAbstractNodes {
