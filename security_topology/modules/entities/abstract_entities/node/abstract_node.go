...
func (abstractNode *AbstractNode) GetNormalNodeFromAbstractNode() (*normal_node.NormalNode, error) {
	...
	case types.NetworkNodeType_FabricPeerNode:
		if fabricPeer, ok := abstractNode.ActualNode.(*nodes.FabricPeerNode); ok {
			return fabricPeer.NormalNode, nil
		}
	case types.NetworkNodeType_FabricOrdererNode:
		if fabricOrderer, ok := abstractNode.ActualNode.(*nodes.FabricOrdererNode); ok {
			return fabricOrderer.NormalNode, nil
		}
	}
