...
// CreateFabricPeerNode 创建 CreateFabricPeerNode
func CreateFabricPeerNode(client *docker.Client, fabricPeerNode *nodes.FabricPeerNode) error {
	...
	peerP2pStartPort := configs.TopConfiguration.FabricConfig.PeerP2pStartPort + fabricPeerNode.Id
	...
	chainCodePort := nat.Port(fmt.Sprintf("%d/tcp", peerChainCodePort))
	operationPort := nat.Port(fmt.Sprintf("%d/tcp", peerOperationPort))
	p2pPort := nat.Port(fmt.Sprintf("%d/tcp", peerP2pStartPort))
	rpcPort := nat.Port(fmt.Sprintf("%d/tcp", peerRpcStartPort))
	...
	portBindings := nat.PortMap{
		...
		chainCodePort: []nat.PortBinding{
			{
				HostIP:   "0.0.0.0",
				HostPort: string(chainCodePort),
			},
		},
