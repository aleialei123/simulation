...
func CreateContainer(client *docker.Client, node *node.AbstractNode) error {
	...
	case types.NetworkNodeType_FabricPeerNode:
		fabricNode, _ := node.ActualNode.(*nodes.FabricPeerNode)
		err = create_apis.CreateFabricPeerNode(client, fabricNode)
		if err != nil {
			return fmt.Errorf("CreateFabricPeerNode err: %w", err)
		}
	case types.NetworkNodeType_FabricOrdererNode:
		fabricNode, _ := node.ActualNode.(*nodes.FabricOrdererNode)
		err = create_apis.CreateFabricOrdererNode(client, fabricNode)
		if err != nil {
			return fmt.Errorf("CreateFabricOrdererNode err: %w", err)
		}
	}
	return nil
}
