...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    orderer_cert orderer orderer1.example.com
...
# "" file that contains the definitions for the sample network. The
...
function createChannel() {
  ...
  scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE $bft_true $ORDERER_NUM $PEER_NUM
}


## Call the script to deploy a chaincode to the channel
function deployCC() {
  scripts/deployCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE $ORDERER_NUM $PEER_NUM
...
function deployCCAAS() {
  scripts/deployCCAAS.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CCAAS_DOCKER_RUN $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE $CCAAS_DOCKER_RUN $ORDERER_NUM $PEER_NUM
...
function networkDown() {
  DOCKER_HOSTS_FILE="docker_hosts_mapping.txt"
  HOSTS_FILE="/etc/hosts"
  # 从 /etc/hosts 中删除 docker_hosts_mapping.txt 中的所有条目
  while IFS= read -r line
  do
    sudo sed -i "/$line/d" $HOSTS_FILE
  done < $DOCKER_HOSTS_FILE

  # 提示删除完成
  echo "已删除 docker_hosts_mapping.txt 中的所有项"
  docker volume ls -q | grep -E '^(orderer|peer|compose)' | while read volume; do
...
#qxp_add
# COMPOSE_FILE_BASE=compose-test-net-qxp.yaml
...
if [ $BFT -eq 1 ]; then
  ...
  #qxp_add
  # COMPOSE_FILE_BASE=compose-bft-test-net-qxp.yaml
fi
