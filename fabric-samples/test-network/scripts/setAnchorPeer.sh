...
local PEER_LISTENPORT_START=5050
...
createAnchorPeerUpdate() {
  ...
  PORT=$((PEER_LISTENPORT_START + ORG))
...
updateAnchorPeer() {
  peer channel update -o localhost:7051 --ordererTLSHostnameOverride orderer1.example.com -c $CHANNEL_NAME -f ${TEST_NETWORK_HOME}/channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile "$ORDERER_CA" >&log.txt
  res=$?
  cat log.txt
