...
for ((i=1;i<=PEER_NUM;i++));do 
  export PEER0_ORG${i}_CA=${TEST_NETWORK_HOME}/organizations/peerOrganizations/org${i}.example.com/tlsca/tlsca.org${i}.example.com-cert.pem
done
setGlobals() {
  local USING_ORG=""
  ...
  #   export CORE_PEER_ADDRESS=localhost:5052
  ...
  #   export CORE_PEER_ADDRESS=localhost:5053
  # else
  #   errorln "ORG Unknown"
  # fi
  if ((USING_ORG <= PEER_NUM)); then
    export CORE_PEER_LOCALMSPID=Org${USING_ORG}MSP
    temp="PEER0_ORG${USING_ORG}_CA"
    export CORE_PEER_TLS_ROOTCERT_FILE=${!temp}
    ...
    export CORE_PEER_ADDRESS=localhost:$((PEER_LISTENPORT_START+USING_ORG))
  fi
