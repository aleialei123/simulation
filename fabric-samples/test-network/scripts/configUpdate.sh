...
. ${TEST_NETWORK_HOME}/scripts/envVar.sh $ORDERER_NUM $PEER_NUM
...
fetchChannelConfig() {
  ...
  set -x
  peer channel fetch config ${TEST_NETWORK_HOME}/channel-artifacts/config_block.pb -o localhost:7051 --ordererTLSHostnameOverride orderer1.example.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
